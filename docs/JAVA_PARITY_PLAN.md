...
- [x] **`cqels-storage-iotdb`** — Apache IoTDB-backed adapter (mirrors Java's `cqels-storage-iotdb`). Lifts the "network database, large surface, no concrete driver" deferral by splitting the integration: a narrow async `IotDbExecutor` trait (`put`, `read_from`, `latest`, `delete_range`, `close`) captures exactly what the SPI needs, and a full-functioning `InMemoryIotDbExecutor` (backed by per-device `BTreeMap<u64, Vec<u8>>`) ships as the default backend. Downstream code wires `iotdb-client-rs::Session` (or any Thrift / native client) in ~50 lines by implementing the trait — kept out of this crate because the Rust IoTDB clients are heavyweight, sync-only with `&mut self`, and pin specific Thrift versions. Same scaffold pattern as `cqels-cdsp` (shape + adapter layer, not a baked-in transport). Storage maps `offset → IoTDB time`, journal device + checkpoint device + reserved key-0 meta row for next-offset recovery across reopens. 14 unit tests (executor-level + backend-level) all green.
...
- [x] **`cqels-cdsp` Rust port (minimal scaffold):** COVESA Vehicle Signal Specification (VSS) signal-ingestion adapter. Provides `VssSignal` + `VssValue` (Bool/Int/Float/String) envelope, a `VssSignalMapper` that produces an `RdfStreamElement` from a signal using a configurable `PredicateStrategy` (`PrefixedPath` or `Slashed`), and an async `VssSignalSource` trait with a reference `ChannelVssSignalSource` backed by `tokio::sync::mpsc`. Network transports (KUKSA.val, COVESA cloud, etc.) are intentionally left for downstream code to plug in via the `VssSignalSource` trait. 16 unit tests (path validation, value variants, serde roundtrip, mapper strategies, channel source lifecycle) + 1 doctest.
