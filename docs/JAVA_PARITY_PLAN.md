...
- [!] **Deferred (3a-layer-5b+):** `TriangleJoinAggregateSpec` and other concrete multi-way specs, Java's prefix/suffix `RangeCache` optimization, `delete_range` (Java itself notes this needs full-tuple TimeIndex), `WindowedJoinAggregateOperator` Reactor envelope, and `ParallelJoinAggregator` Tier-2 batch recompute. ~1.5K Java lines remaining.
