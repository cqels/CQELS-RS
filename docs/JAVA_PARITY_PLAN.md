...
- [!] **Deferred (3a-layer-3+):** `MultiWayJoinState` / `JoinGraph` / `FactorizedViewPlan` (F-IVM planner), `WindowedJoinAggregateOperator`, plus the 6 concrete aggregate-spec classes (`CountSpec`, `SumLeftSpec`, `SumRightSpec`, `SumProductSpec`, `MultiWayAggregateSpec`, `TriangleJoinAggregateSpec`). ~4K Java lines remaining; will be split into further layered PRs.
