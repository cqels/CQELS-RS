...
- [!] **Deferred (3a-layer-4):** `MultiWayJoinState` (executor over a planned graph), `WindowedJoinAggregateOperator`, the 6 concrete aggregate-spec classes (`CountSpec`, `SumLeftSpec`, `SumRightSpec`, `SumProductSpec`, `MultiWayAggregateSpec`, `TriangleJoinAggregateSpec`), and `ParallelJoinAggregator`. ~3K Java lines remaining.
