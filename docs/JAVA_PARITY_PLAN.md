...
- [!] **Deferred (3a-layer-5c+):** Java's prefix/suffix `RangeCache` optimization, `delete_range` (Java itself notes this needs full-tuple TimeIndex), `WindowedJoinAggregateOperator` Reactor envelope, and `ParallelJoinAggregator` Tier-2 batch recompute. ~1.2K Java lines remaining.
