...
- [!] **Deferred (3a-layer-5+):** `MultiWayJoinState` (the recursive F-IVM N-way executor), `MultiWayAggregateSpec` + `TriangleJoinAggregateSpec` (multi-way spec hierarchy), `WindowedJoinAggregateOperator` Reactor-style envelope, and `ParallelJoinAggregator` (Tier-2 batch recompute). ~2K Java lines remaining for the full multi-way pipeline.
