...
- [!] **Deferred (3a-layer-2 onward):** `WindowJoinState` (pairwise join), `MultiWayJoinState` / `JoinGraph` / `FactorizedViewPlan` (F-IVM planner), `WindowedJoinAggregateOperator`, plus the 6 aggregate-spec classes. ~5K Java lines across 11+ files; will be split into further layered PRs.
