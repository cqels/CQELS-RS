...
| Metric | Value |
|---|---|
| **Total tests** | **1,827 passing** (default features) + 43 optional-feature tests = **~1,870** |
| **Failures** | **0** |
...
| Tests | `cargo test --workspace` | ✅ 1827 passed, 4 ignored |
...
| `cqels-mcp` | **161** | MCP tool/prompt/resource surface + JSON-RPC stdio transport | `cqels-mcp` |
...
|---|:---:|:---:|
| `cargo build --workspace` | ✅ | 1,827 passed, 4 ignored |
...
```
Latest full workspace: passed=1827 failed=0 ignored=4
```
...
- **Phase 3c** (external integration): ✅ complete — MCP tool/resource
  surface, `cqels-cdsp` with real KUKSA gRPC client
