...
**Latest delta (2026-07-12):** the default `cqels-mcp` server now wires
...
default `literal`); `cqels://engine/status` now reports Java alpha.10
liveness/runtime fields while leaving `rdfStore.persistent` false until true
...
typed integer literals. `cargo test -p cqels-mcp` now passes **214** unit
...
| Metric | Value |
|---|---|
| **Total tests** | **1,889 passing** (default features) + 43 optional-feature tests = **~1,932** |
| **Failures** | **0** |
...
| Tests | `cargo test --workspace` | ✅ 1889 passed, 4 ignored |
...
| `cqels-mcp` | **214** | MCP tool/prompt/resource surface + JSON-RPC stdio / HTTP transports | `cqels-mcp` |
...
|---|:---:|:---:|
| `cargo build --workspace` | ✅ | 1,889 passed, 4 ignored |
...
```
Latest full workspace: passed=1889 failed=0 ignored=4
```
